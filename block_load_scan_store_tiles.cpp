// circle --verbose -O3 -std=c++20 -I../ubu-tot -sm_60 block_load_scan_store_tiles.cpp -lcudart -lfmt -o block_load_scan_store_tiles
...
ubu::cuda::event load_scan_store_tiles_after(ubu::cuda::device_executor gpu, const ubu::cuda::event& before, std::span<const int> input, std::span<int> result, Function op)
{
  using namespace ubu;
  ...
  constexpr size_t num_tiles_per_block = 2;
  constexpr size_t max_tile_size = max_num_elements_per_thread * block_size;
  size_t num_blocks = ceil_div(num_elements, num_tiles_per_block * max_tile_size);

  // kernel configuration
  std::pair kernel_shape(block_size, num_blocks);

  // 48 registers / 404.341 GB/s ~ 92% peak bandwidth
                            ...
                            [=](ubu::int2 idx)
  {
    ...
    for(int tile_idx = 0; tile_idx < num_tiles_per_block; ++tile_idx)
    {
      ...
      size_t block_begin = blockIdx.x * max_tile_size * num_tiles_per_block + tile_idx * max_tile_size;
      ...
      // Incorporate the carry-in into the first's thread's input
      if(threadIdx.x == 0)
      {
        thread_data[0] = op(carry_in, thread_data[0]);
      }

      // XXX do we require a syncthreads here? is there a race on carry_in?

      // Scan across the block, get the next tile's carry-in
      BlockScan(temp_storage.scan).InclusiveScan(thread_data, thread_data, op, carry_in);
      __syncthreads();
...
void test_load_scan_store_tiles_after(std::size_t n)
{
  ...
  load_scan_store_tiles_after(ex, before, std::span(input), std::span(result), std::plus()).wait();
...
void test_correctness(std::size_t max_size, bool verbose = false)
{
  for(auto sz: test_sizes(max_size))
  {
    ...
    test_load_scan_store_tiles_after(sz);
...
double test_performance(std::size_t size, std::size_t num_trials)
{
  ...
  // warmup
  load_scan_store_tiles_after(ex, before, input_view, result_view, std::plus{});
